...
pub mod T {
    ...
    impl DB {
        ...
        pub fn calc_converted(&mut self, pos: &Pos, p: H::Player) -> CalcResult {
        ...
        // Gibt Some(Togre) zurück, wenn die gegebene Stellung bereits bekannt oder gewonnen ist. Sonst None
        fn get_known_togre(&mut self, pos: &Pos, p: &Player) -> Option<Togre> {
            // Ist pos in der DB?
            if let Some(saved) = self.get(&pos, &p) {
                Some(saved)
            } else if let Some(won) = pos.won() {
                Some(won.togre())
            } else { None }
        }
        ...
        fn i(&mut self, pos: &Pos, p: &Player) -> Togre {
            ...
            let mut remis = true; // Ist pos final remis? Wir gehen davon aus, bis das Gegenteil bewiesen wird.
            ...
            for f in startfelder {
                ...
                let (mut zielfeld, mut zielpos);

                // Ich könnte einen Schritt machen…
                zielfeld = f + (p.m() * 6);
                // …wenn vor dem Startfeld niemand steht:
                if ! (pos.of(&Player::X).contains(&zielfeld) || pos.of(&Player::O).contains(&zielfeld)) {
                    // Wenn ich einen Schritt machen kann, ist pos schonmal nicht final remis.
                    remis = false;
                    // Dann wenden wir das an und erhalten die neue Stellung.
                    zielpos = pos.zug_anwenden(Zug { from: *f, to: zielfeld}, p);
                    // Diese fragen wir zunächst in der DB und nach Finalität ab (in get_known_togre)…
                    if let Some(togr) = self.get_known_togre(&zielpos, &p.not()) {
                    ...
                    } else { later.push(zielpos); };

                    // Außerdem könnte ich auch einen Sprung machen.
                    zielfeld = f + (p.m() * 12);
                    // Dazu müsste (zusätzlich zu der Bedingung oben, innerhalb derer wir uns immernoch befinden) das Feld vor `ziel_schritt` (`ziel_sprung`) frei sein…
                    if ! (pos.of(&Player::X).contains(&zielfeld) || pos.of(&Player::O).contains(&zielfeld)) &&
                ...
                // …und schräg rechts davor müsste der Gegner stehen.
                    pos.of(&p.not()).contains(&zielfeld)
                {
            ...
            // Zwischenstand: Wir haben jetzt über alle Startfelder iteriert und geschaut, welche Zugoptionen es dort gibt. Bei jeder Zugoption, die möglich war, haben wir remis=false gesetzt und damit gesagt: Es gibt mindestens eine Zugoption. pos ist folglich nicht final remis. (Das fehlte vormals und hat mich zwei Tage sehr nerviger Fehlersuche gekostet.)
            // Wenn die aus der Zugoption resultierende Folgestellung bekannt war, dann sind wir (wenn sie positiv war) schon rausgeflogen oder haben (wenn sie neutral war) neutral=true gesetzt. Folgestellungen, die noch unbekannt waren, sind in later gespeichert. Jetzt iterieren wir über die unbekannten Folgestellungen und berechnen sie. Das kann länger dauern…
            for late in later {
            ...
            // Zwischenstand:
                // Wenn wir irgendwann eine positive Folgestellung gefunden hatten, sind wir schon längst rausgeflogen. Dann wären wir also nicht hier.
                // Wenn wir eine neutrale gefunden hatten, ist das in neutral=true vermerkt.
                // Wenn also neutral=false ist, dann ist pos garantiert negativ :'-(
                // Es sei denn: Wenn noch remis=true ist, dann heißt das, wir haben noch gar keine Folgestellung gefunden. Folglich ist pos final remis.
            if neutral || remis { Togre::R } else { p.not().togre() }
        }
